...
def track_objects_in_lines(video_path, model_path, lines, frame_skip, recording_start_time , video_identifier):
...
def select_and_process_videos():
    """Open a Tkinter GUI to select two videos and process them with threading."""
    root = tk.Tk()
    root.withdraw()  # Hide the root window

    # Select two video files
    video_path1 = filedialog.askopenfilename(title="Select First Video")
    video_path2 = filedialog.askopenfilename(title="Select Second Video")

    if not video_path1 or not video_path2:
        print("Both video files must be selected.")
        return

    # Parameters
    model_path = "yolov8n.pt"  # Path to the YOLO model
    lines = [((100, 600), (800, 600)), ((150, 800), (850, 800))]  # Example lines
    frame_skip = 5  # Skip every 5 frames
    recording_start_time = datetime.now()
    def select_directory():
        return filedialog.askdirectory(title="Select Directory to Save Processed Data")

    # Start threads for processing videos
    Thread(target=track_objects_in_lines,
           args=(video_path1, model_path, lines, frame_skip, recording_start_time, "Video 1")).start()

    Thread(target=track_objects_in_lines,
           args=(video_path2, model_path, lines, frame_skip, recording_start_time, "Video 2")).start()
